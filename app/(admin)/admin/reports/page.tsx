...
import { BarChart3, Calendar, DollarSign, Download, Eye, FileText, FileSpreadsheet, Filter, RefreshCw, TrendingUp, Users, Building, GraduationCap, BookOpen, Target } from 'lucide-react'
...
export default function ReportsPage() {
  ...
  const [signalsChartData, setSignalsChartData] = useState<ChartData[]>([])
  const [coursesChartData, setCoursesChartData] = useState<ChartData[]>([])
  ...
  const fetchReportData = async () => {
    setIsLoading(true)
    try {
      const response = await fetch(`/api/admin/reports/overview?dateRange=${dateRange}`)
      if (!response.ok) {
  ...
  const fetchChartData = async (type: string) => {
    try {
      const response = await fetch(`/api/admin/reports/charts?type=${type}&dateRange=${dateRange}`)
      if (!response.ok) {
  ...
  const fetchAllChartData = async () => {
    const [revenue, users, signals, courses] = await Promise.all([
      fetchChartData('revenue'),
      fetchChartData('users'),
      fetchChartData('signals'),
      fetchChartData('courses')
    ])
    
    setRevenueChartData(revenue)
    setUserChartData(users)
    setSignalsChartData(signals)
    setCoursesChartData(courses)
  }
  ...
  return (
    <div className="space-y-6 sm:space-y-8 p-4 sm:p-6 lg:p-8">
        ...
        <Card className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-lg transition-all duration-300 group">
          <CardContent className="p-8">
            <div className="flex flex-col space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <p className="text-sm font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Signal Hit Rate</p>
                  <p className="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white mt-2">{reportData?.signals.hitRate || 0}%</p>
                </div>
              ...
              <div className="pt-2 border-t border-gray-100 dark:border-gray-700">
                <p className="text-sm text-gray-600 dark:text-gray-400 font-medium">
                  {formatNumber(reportData?.signals.subscribers || 0)} subscribers
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-lg transition-all duration-300 group">
          <CardContent className="p-8">
            <div className="flex flex-col space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <p className="text-sm font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Course Enrollments</p>
                  <p className="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white mt-2">{formatNumber(reportData?.courses.enrollments || 0)}</p>
                </div>
              ...
              <div className="pt-2 border-t border-gray-100 dark:border-gray-700">
                <p className="text-sm text-gray-600 dark:text-gray-400 font-medium">
                  {formatNumber(reportData?.courses.total || 0)} courses available
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-lg transition-all duration-300 group">
          <CardContent className="p-8">
            <div className="flex flex-col space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <p className="text-sm font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">Forecasts</p>
                  <p className="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white mt-2">{formatNumber(reportData?.forecasts.total || 0)}</p>
                </div>
              ...
              <div className="pt-2 border-t border-gray-100 dark:border-gray-700">
                <p className="text-sm text-gray-600 dark:text-gray-400 font-medium">
                  {formatNumber(reportData?.forecasts.likes || 0)} likes
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
        ...
        <Card className="bg-theme-card border-theme-border">
          <CardHeader>
            <CardTitle className={textHierarchy.sectionHeading(isDarkMode)}>Signals Performance</CardTitle>
            <CardDescription className={textHierarchy.subheading()}>Signals published and engagement</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={signalsChartData}>
                <CartesianGrid strokeDasharray="3 3" stroke={isDarkMode ? '#374151' : '#e5e7eb'} />
                <XAxis dataKey="name" stroke={isDarkMode ? '#9ca3af' : '#6b7280'} />
                <YAxis stroke={isDarkMode ? '#9ca3af' : '#6b7280'} />
                <Tooltip 
                ...
                />
                <Line type="monotone" dataKey="published" stroke="#8b5cf6" strokeWidth={3} name="Published" />
                <Line type="monotone" dataKey="likes" stroke="#f59e0b" strokeWidth={3} name="Likes" />
                <Line type="monotone" dataKey="comments" stroke="#10b981" strokeWidth={3} name="Comments" />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card className="bg-theme-card border-theme-border">
          <CardHeader>
            <CardTitle className={textHierarchy.sectionHeading(isDarkMode)}>Course Enrollments</CardTitle>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
        ...
        <Card className="bg-theme-card border-theme-border">
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                <span className={`text-sm font-medium ${textHierarchy.cardDescription()}`}>Course Revenue</span>
                <span className={`text-sm ${textHierarchy.metaText(isDarkMode)}`}>{formatCurrency(reportData?.courses.revenue || 0)}</span>
              </div>
              <div className="pt-4 border-t border-theme-border-secondary space-y-2">
                <Button 
                  variant="outline" 
        ...
        <Card className="bg-theme-card border-theme-border">
          <CardHeader>
            <CardTitle className={textHierarchy.sectionHeading(isDarkMode)}>Signals Report</CardTitle>
            <CardDescription className={textHierarchy.subheading()}>Signal performance metrics</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className={`text-sm font-medium ${textHierarchy.cardDescription()}`}>Total Signals</span>
                <span className={`text-sm ${textHierarchy.metaText(isDarkMode)}`}>{formatNumber(reportData?.signals.total || 0)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className={`text-sm font-medium ${textHierarchy.cardDescription()}`}>Published</span>
                <span className={`text-sm ${textHierarchy.metaText(isDarkMode)}`}>{formatNumber(reportData?.signals.published || 0)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className={`text-sm font-medium ${textHierarchy.cardDescription()}`}>Hit Rate</span>
                <span className={`text-sm ${textHierarchy.metaText(isDarkMode)}`}>{reportData?.signals.hitRate || 0}%</span>
              </div>
              <div className="flex justify-between items-center">
                <span className={`text-sm font-medium ${textHierarchy.cardDescription()}`}>Subscribers</span>
                <span className={`text-sm ${textHierarchy.metaText(isDarkMode)}`}>{formatNumber(reportData?.signals.subscribers || 0)}</span>
              </div>
              <div className="pt-4 border-t border-theme-border-secondary space-y-2">
                <Button 
                  variant="outline" 
                  size="sm" 
                  onClick={() => exportReport('signals', 'excel')}
                  disabled={isExporting === 'signals-excel'}
                  className="w-full border-theme-border hover:bg-theme-bg-secondary"
                >
                  {isExporting === 'signals-excel' ? (
                    <RefreshCw className="h-4 w-4 mr-2 animate-spin" />
                  ) : (
                    <FileSpreadsheet className="h-4 w-4 mr-2" />
                  )}
                ...
                <Button 
                  variant="outline" 
                  size="sm" 
                  onClick={() => exportReport('signals', 'pdf')}
                  disabled={isExporting === 'signals-pdf'}
                  className="w-full border-theme-border hover:bg-theme-bg-secondary"
                >
                  {isExporting === 'signals-pdf' ? (
                    <RefreshCw className="h-4 w-4 mr-2 animate-spin" />
                  ) : (
                    <FileText className="h-4 w-4 mr-2" />
                  )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <span className={`text-sm ${textHierarchy.cardDescription()}`}>Signals Report</span>
                <span className={`text-xs ${textHierarchy.metaText(isDarkMode)}`}>Excel</span>
              </Button>
              <Button 
                variant="outline" 
            ...
            <div className="space-y-2">
              <Button 
                variant="outline" 
                onClick={() => exportReport('courses', 'excel')}
                disabled={isExporting === 'courses-excel'}
                className="h-16 w-full flex flex-col items-center justify-center space-y-1 border-theme-border hover:bg-theme-bg-secondary"
              >
                {isExporting === 'courses-excel' ? (
                  <RefreshCw className="h-5 w-5 animate-spin" />
                ) : (
                  <FileSpreadsheet className="h-5 w-5" />
                )}
                <span className={`text-sm ${textHierarchy.cardDescription()}`}>Courses Report</span>
                <span className={`text-xs ${textHierarchy.metaText(isDarkMode)}`}>Excel</span>
              </Button>
              <Button 
                variant="outline" 
                onClick={() => exportReport('courses', 'pdf')}
                disabled={isExporting === 'courses-pdf'}
                className="h-16 w-full flex flex-col items-center justify-center space-y-1 border-theme-border hover:bg-theme-bg-secondary"
              >
                {isExporting === 'courses-pdf' ? (
                  <RefreshCw className="h-5 w-5 animate-spin" />
                ) : (
                  <FileText className="h-5 w-5" />
                )}
