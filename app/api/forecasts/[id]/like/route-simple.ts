...
// POST /api/forecasts/[id]/like - Like/unlike a forecast (Simple version)
export async function POST(
  request: NextRequest,
  { params }: { params: { id: string } }
) {
  try {
    console.log('=== SIMPLE LIKE API CALL ===')
    console.log('Request URL:', request.url)
    
    // Get authenticated user (simple version)
    const user = await getAuthenticatedUserSimple(request)
    console.log('Simple authenticated user:', user)
    ...
    console.error('Simple like API error:', error)
    return NextResponse.json({ 
